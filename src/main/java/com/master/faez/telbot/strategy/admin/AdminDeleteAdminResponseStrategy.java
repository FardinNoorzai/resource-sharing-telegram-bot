...
@Component
public class AdminDeleteAdminResponseStrategy implements ResponseStrategy {
    @Autowired
        ...
        try{
            ...
            if(user != null) {
                if(user.getUserRole().equals(USER_ROLE.ADMIN)){
                    applicationEventPublisher.publishEvent(new DeleteAdminEvent(this,user.getId()));
                    applicationEventPublisher.publishEvent(new ProcessedMessage(this,null,null, List.of("User "+ user.getName() + " was deleted from the list of admins"),userSession));
                }else{
                    applicationEventPublisher.publishEvent(new ProcessedMessage(this,null,null, List.of("This user is not an admin in the bot"),userSession));
                }
            }else{
